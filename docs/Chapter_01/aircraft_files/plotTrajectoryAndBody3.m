function plotTrajectoryAndBody3(h_fig,vXe,vYe,vZe,vQuat,scale_factor,step,theView,varargin)
%
...
if (nVarargs == 1)
    ...
    UVWe = varargin{1};
    % sanity check
    if (length(UVWe(:,1))~=length(vXe))
        disp('  Error:');
        disp('      Wrong dimensions of CG velocity vectors. Check the size.');
        return;
    end
    bPlotVelocity = 1;
end
